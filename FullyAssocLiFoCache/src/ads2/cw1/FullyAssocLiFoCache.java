...
class FullyAssocLiFoCache implements Cache {
    ...
    // 4 data structures are needed
    // - One for the cache storage ( a 1D array of Strngs)
    private int[] cache_storage;
    // - One to manage locations in the cache ( a stack of Integers)
    private Stack<Integer> location_stack;
    // And because the cache is Fully Associative:
    // - One to translate between memory addresses and cache locations ( a HashMap)
    private Map<Integer,Integer> address_to_cache_loc;
    // - One to translate between cache locations and memory addresses ( a HashMap)
    ...
    FullyAssocLiFoCache(int cacheSize, int cacheLineSize) {
        ...
        cache_storage = new Int[CASHE_SZ];
        Stack<Integer> location_stack; = new Stack<Integer>();;
        Map address_to_cache_loc; = new HashMap<Integer,Integer>();
        Map cache_loc_to_address; = new HashMap<Integer,Integer> ();
